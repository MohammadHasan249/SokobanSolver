    ...
    def print_open(self):
        ...
        if len(self.open) == 1:
            print("   <S{}:{}:{}, g={}, h={}, f=g+h={}>".format(
                self.open[0].state.index, self.open[0].state.action,
                self.open[0].state.hashable_state(), self.open[0].gval,
                self.open[0].hval, self.open[0].gval + self.open[0].hval),
                end="")
        else:
            for nd in self.open:
                print("   <S{}:{}:{}, g={}, h={}, f=g+h={}>".format(
                    nd.state.index, nd.state.action, nd.state.hashable_state(),
                    nd.gval, nd.hval, nd.gval + nd.hval), end="")
        print("}")
...
class SearchEngine:
    ...
    # An object of class Open is used to represent the search frontier.
    # An Open object organizes the search frontier in a way that is appropriate for a given search strategy.
    ...
    def set_strategy(self, s, cc='default'):
        if not s in ['depth_first', 'breadth_first', 'ucs', 'best_first',
                     'astar', 'custom']:
            print('Unknown search strategy specified:', s)
            print(
                "Must be one of 'depth_first', 'ucs', 'breadth_first', 'best_first', 'custom' or 'astar'")
    ...
    def init_search(self, initState, goal_fn, heur_fn=_zero_hfn,
                    fval_function=_fval_function):
        ...
        # heur_fn(s) is a function that returns a heuristic value for the state s.
        # This function will only be used if your search engine has been instantiated to be
        # a heuristic search (e.g., best first).

        # fval_fn(sNode) deﬁnes f-values for states. This function will only be used by your
        # search engine if it has been instantiated to execute a custom search. Note that this
        # function takes in an sNode and that an sNode contains not only a state but additional
        # measures of the state (e.g., a gval). The function will use the variables that are provided
        # in order to arrive at an f-value calculation for the state contained in the
    ...
    def search(self, timebound=None, costbound=None):
        ...
        # timebound is a bound on the amount of time your code will execute the search.
        # Once the run time exceeds the time bound, the search will stop;
        # if no solution has been found, the search will return False.

        # costbound is an optional bound on the cost of each state s that is explored.
        # The parameter costbound should be a 3-tuple (g bound,h bound,g + h bound).
        # If a node's g val is greater than g bound, h val is greater than h bound,
        # or g val + h val is greater than g + h bound, that node will not be expanded.
        # You will use costbound to implement pruning in both of the anytime searches described below.
        ...
        goal_node = self._searchOpen(self.goal_fn, self.heur_fn,
                                     self.fval_function, costbound)
    ...
    def _searchOpen(self, goal_fn, heur_fn, fval_function, costbound):
        ...
        while not self.open.empty():
            ...
            if self.trace:
                print(
                    "   TRACE: Next State to expand: <S{}:{}:{}, g={}, h={}, f=g+h={}>".format(
                        node.state.index, node.state.action,
                        node.state.hashable_state(), node.gval, node.hval,
                        node.gval + node.hval))
            ...
            if self.trace:
                if self.cycle_check == _CC_FULL:
                    print("   TRACE: CC_dict gval={}, node.gval={}".format(
                        self.cc_dictionary[node.state.hashable_state()],
                        node.gval))
            # END TRACING

            if self.cycle_check == _CC_FULL and self.cc_dictionary[
                 node.state.hashable_state()] < node.gval:
                continue
            ...
            if self.trace:
                ...
                for ss in successors:
                    print("<S{}:{}:{}, g={}, h={}, f=g+h={}>, ".format(
                        ss.index, ss.action, ss.hashable_state(), ss.gval,
                        heur_fn(ss), ss.gval + heur_fn(ss)), end="")
                print("}")
            # END TRACING
            ...
            for succ in successors:
                ...
                if prune_succ:
                    ...
                    if self.trace > 1:
                        print(
                            " TRACE: Successor State pruned by cycle checking")
                        print("\n")
                        # END TRACING
                    continue
                ...
                if costbound is not None and (succ.gval > costbound[0] or
                                              ...
                                              succ.gval + succ_hval > costbound[
                                                  2]):
                    self.cost_bound_pruned = self.cost_bound_pruned + 1
                    if self.trace > 1:
                        print(
                            " TRACE: Successor State pruned, over current cost bound of {}",
                            costbound)
                        print("\n")
                    continue

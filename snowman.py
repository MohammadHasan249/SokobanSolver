...
class SnowmanState(StateSpace):
    ...
    def __init__(self, action, gval, parent, width, height, robot, snowballs,
        ...
        # snowballs: positions for each snowball (or stack of snowballs) as keys of a dictionary.
        # Each position is an (x, y) tuple.
        # The value of each key is the index for that snowball's size (see below).
        # Some values denote stacks of snowballs at a given location as well.

        # obstacles: locations of all of the obstacles (i.e. walls) on the board.
        # Obstacles, like robots and snowballs, are also tuples of (x, y) coordinates.

        # destination: the target destination for the snowperson: a tuple (x, y), that denotes the desired position
        # for the completed snowperson.

        # sizes: contains the key, value pairs that indicate snowball sizes or the presence of a snowball stack.
        # The possible values are: ’b’ for a big snowball, ’m’ for a medium snowball, and ’s’ for a small one.
        # A ’G’ denotes a completed snowperson.
        # In addition, note that there are values to indicate stacks of snowballs on the board:
        # ’A’ represents a medium snowball atop big one, ’B’ represents a small snowball atop big one and
        # ’C’ represents a small snowball atop medium one.
        # See Figure 2 for snowballs as they are represented by the ASCII visualizer you have been provided.
        ...
        self.snowball_sizes = {0: 'b', 1: 'm', 2: 's', 3: 'A', 4: 'B', 5: 'C',
                               6: 'G'}

    def successors(self):

        # This function generates a list of SnowmanStates that are successors to a given SnowmanState.
        # Each state will be annotated by the action that was used to arrive at the SnowmanState up, down, left, right.
        ...
        for direction in (UP, RIGHT, DOWN, LEFT):
            ...
            if new_location in self.snowballs:  # if the location we're going to is where there's a snowball
                new_snowball_location = direction.move(
                    new_location)  # move the snowball

                # snowball out of bounds?
                if new_snowball_location[0] < 0 or new_snowball_location[
                    0] >= self.width:
                    continue
                if new_snowball_location[1] < 0 or new_snowball_location[
                    1] >= self.height:
                    continue
                if new_snowball_location in self.obstacles:
                    continue
                if self.snowball_sizes[new_snowballs[
                    new_location]] == 'G':  # can't move a complete Snowman
                    continue
                ...
                if new_snowball_location in new_snowballs:  # if the new snowball location is where there's a snowball
                    if self.snowball_sizes[
                        new_snowballs[new_snowball_location]] == 'b' and \
                            self.snowball_sizes[
                                new_snowballs[new_location]] == 'm':
                        index = 3  # will transition to A formation of snowballs
                    elif self.snowball_sizes[
                        new_snowballs[new_snowball_location]] == 'm' and \
                            self.snowball_sizes[
                                new_snowballs[new_location]] == 's':
                        ...
                        new_snowballs[new_snowball_location]] == 'A' and \
                            self.snowball_sizes[
                                new_snowballs[new_location]] == 's':
            ...
            new_state = SnowmanState(action=direction.name,
                                     gval=self.gval + transition_cost,
                                     parent=self,
    ...
    def hashable_state(self):
        ...
        # This is a function that calculates a unique index to represents a particular SnowmanState.
        # It is used to facilitate path and cycle checking.
    ...
    def state_string(self):
        ...
        for snowball in self.snowballs:
            map[snowball[1]][snowball[0]] = self.snowball_sizes[
                self.snowballs[snowball]]
...
def snowman_goal_state(state):
  ...
  """
    for snowball in state.snowballs:
        if (state.snowball_sizes[state.snowballs[snowball]] == 'G' and snowball[
            1] == state.destination[1] and snowball[0] == state.destination[0]):
            # means a complete snowman is on the board and in the right spot
            return True
    return False
